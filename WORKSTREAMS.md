...
**Prime directive:** **read-only.** It reuses the existing engines as the single source of truth (no second scoring engine, no recomputation, no confidence reinterpretation), never writes to the knowledge base, and never implies a product has been validated or selected. Its only output is the gitignored `executive-ui/dist/`.

**Does not modify:** backend scoring, evidence parsing, monitoring state, or any knowledge-base record.
