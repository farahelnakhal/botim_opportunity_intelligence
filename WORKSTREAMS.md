...
**Purpose:** a human-reviewed pipeline that turns BOTIM merchant feedback (surveys, interviews, concept tests) into traceable Part A evidence *proposals*. v1 (Phase 1 + 2 + 3, delivered) implements research campaigns, versioned research guides, pseudonymous participants, consent/privacy gating, manual and CSV-bulk response ingestion, text transcript ingestion, deterministic redaction, withdrawal/retention/deletion, and AI-assisted extraction of structured observations — the model may only *propose* observations (never approve, score, or finalize); every one is created `pending_review` and nothing here is authoritative. Later phases add reviewer approval/rejection, evidence candidates, approved findings, campaign aggregation, and a Part A proposal preview.
