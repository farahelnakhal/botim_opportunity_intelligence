...
**Purpose:** a human-reviewed pipeline that turns BOTIM merchant feedback (surveys, interviews, concept tests) into traceable Part A evidence *proposals*. v1 (Phase 1 + 2 + 3 + 4, delivered) implements research campaigns, versioned research guides, pseudonymous participants, consent/privacy gating, manual and CSV-bulk response ingestion, text transcript ingestion, deterministic redaction, withdrawal/retention/deletion, AI-assisted extraction of structured observations (the model may only *propose*), and the full human-governed review workflow: observation review/edit/approve/reject/merge, evidence candidates, and immutable approved Merchant Voice findings with deterministic strength bands and campaign-level analysis. **An approved Merchant Voice finding is still NOT authoritative Part A evidence** — nothing here writes there, changes Part B scores, or changes assumptions. Later phases add a Part A proposal preview and Copilot integration.
...
**Write boundary:** Merchant Voice never writes directly into authoritative Part A evidence records, never mints Part A evidence IDs, and never directly changes Part B scores or assumptions. Any future Part A evidence proposal is generated and stored inside `merchant-voice/data/mv.db` for preview only; export of real merchant-derived findings to the knowledge base requires a separate privacy/redaction/quote-permission review, human reviewer approval, and explicit Workstream A acceptance.
...
**Shared contract:** `shared/contracts/merchant-voice-api.schema.md` (Phase 1 subset only; later phases are documented there as an explicitly-labelled, not-yet-active future roadmap).
