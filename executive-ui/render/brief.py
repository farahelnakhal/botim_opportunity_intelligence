...
def render(model):
    ...
    intro = ('<p class="lede">Executive briefs are consumed from the brief generator '
             '(<code>impact/brief.py</code> / <code>impact-cli brief</code>, and committed recommendation '
             'docs) — this view does not re-derive them. Each brief states a decision requested; none '
             'asserts that a product has been validated or selected.</p>')
    if not have:
