...
|---|---|---|
| `ANTHROPIC_API_KEY` | *(unset)* | When set, analysis is generated by Claude. When absent, a deterministic offline **scaffold** is returned instead (clearly labelled) — a frame to run the analysis yourself. |
| `BOTIM_ANALYSIS_MODEL` | `claude-sonnet-5` | Model used for generation. |
| `BOTIM_ANTHROPIC_BASE_URL` | `https://api.anthropic.com` | Override the API base URL if needed. |
