...
class TestRender(unittest.TestCase):
    ...
    def test_feed_has_typed_items(self):
        ...
        self._common(html)
        # approved changes flow only through the impact workflow; the feed shows upstream signals
        self.assertIn("evidence-impact workflow", html)
        self.assertIn("not approved impacts", html)

    def test_proposal_is_readonly_no_fake_button(self):
        html = proposal.render(self.m)
        self._common(html)
        self.assertIn("intentionally read-only", html)
        self.assertIn("apply-impact --approver", html)   # approval is a CLI action, named
