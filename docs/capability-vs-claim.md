...
|---|---|---|---|
| **Multi-language, multi-platform social scraping** — pulls customer opinion from Reddit / social / app-store reviews in 4 languages | **Not built (methodology-doc-only).** No social/app-store/Reddit integration; no language-specific search. Live research is a single general web-search adapter. | `shared/research/providers.py` = `BraveSearchProvider` only (sends `q`+`count`, no language param); `shared/research/retrieval.py` = generic http(s) fetch, zero `lang`/`hl`/`locale` handling. `customer-intelligence/tools/` = only `conformance_check.py` (a format validator). The Reddit/social/multilingual language lives in `WORKSTREAMS.md:20`, `customer-intelligence/commands/example-commands.md:14`, `customer-intelligence/guides/source-discovery.md` — operator instructions (they list *five* languages, not four). | Per-platform adapters (Reddit/app-store/etc.) behind the `providers.py` seam; language/locale params on search + fetch; provenance + tests per source type. Substantial. **Planned: R9a** (Apple RSS + Reddit adapters + source tiering), **R9b** (multi-language querying), **R9c** (non-English source content — deferred). |
| **Verified-source stats → TAM/SAM/SOM computation** | **Partly built (C1 calculators shipped; "verified-source" binding is C2, not built).** A deterministic `market_sizing` calculator (top-down TAM/SAM/SOM, inputs→formula→outputs shown) now exists and is exposed at request time / chat. What is still **not** built: pulling the input figures from *verified* live sources — today the inputs are user/assumption-provided (labelled illustrative), not extracted-and-corroborated web stats. | C1: `shared/calculators/{base,calculators,render,store}.py` — `market_sizing` (+ bottom-up), unit economics, break-even, payback, `payments_take`; `/calculators/*` routes; copilot `run_calculator`. Offline engine unchanged: `opportunity-intelligence/tools/opportunity_engine/{scoring,commercial,…}.py`. Research sources carry recorded quality signals + freshness + dedup (`shared/research/runner.py`), reviewed as candidates — a curated **source-tier** layer (T1–T4) arrives with R9a. | **C1 done.** Remaining: **C2** — extract figures from sources (PR3 exact-substring discipline), corroborate against ≥2 T1/T2 sources (R9a tiers), feed the C1 calculator via the reserved `source_id` input, results candidate/preliminary and human-reviewed. |
| **Merchant interview — auto-suggested questions from gaps; answers update KB + models** | **Not built as described (manual workflow).** Campaigns/guide questions are human-authored; no gap-detection→question generation. Approved answers become **candidate** evidence needing human review, never an automatic model/score update. | `merchant-voice/app/campaigns.py:create()` takes `research_questions` from request data; `merchant-voice/app/guides.py` inserts human-entered questions validated against a taxonomy. Approved findings export synthetic-only to `knowledge-base/customer-evidence/merchant-voice-candidates/` (never `records/`, never EV ids); copilot reads them **read-only** via `merchant-voice/app/published_query.py`. No path writes the scoring engine or opportunity records. | A gap→question generator (consume assumption-register/evidence-gaps, propose questions); and — only if desired — a reviewed promotion path, still ending at human approval, never auto-writing committed scores. **Planned: R10.** |
| **News updates — competitor/customer tracking + notifications.** *(The original claim also said "auto-updating models/data" — that phrasing is **mis-worded**; see the cell to the right.)* | **Built (the real target); the "auto-update" phrasing is a non-goal, not a gap.** Tracking + notifications exist. Monitoring/R6 produce **candidate evidence + preliminary workspace versions + notifications** and stop there, pending human review — which is exactly the intended behavior. | Built: `intelligence-monitoring/tools/monitoring_engine/` (KB-diff `kbwatch`, `events`, `alerts`, `digest`) + one offline-injectable external adapter (`adapter_regulator.py`); `executive-ui/api/monitoring_runner.py` (manual R4a runs → `MEVT-` events, runtime); R6 scheduled re-run + email (`shared/workspace/`, tick in `executive-ui/api/server.py`, `shared/email/`). The **only** writer to committed `knowledge-base/opportunity-scores/` is `impact/apply.py:apply_impact(ref, approver)` — **requires `--approver`, CLI-only** (`impact/cli.py`); nothing in monitoring/R6 imports it. | **N/A — not a gap, no phase planned.** "Auto-update authoritative data" is a **deliberate non-goal**: the read-only-KB + human-approval invariant is founding. Closing it would mean *removing* an invariant, not adding a feature. The claim was reworded to its real target above. **Notification ≠ authoritative auto-update.** (Confirmed 2026-07-20.) |
...
- **"Run transparent analytical calculations"** (product-context vision) — the
  offline engine's calculations run on committed inputs, AND as of **C1**
  deterministic calculators (market sizing, unit economics, break-even,
  payback, payments-take) are now **exposed at chat/request time** with fully
  shown working and no LLM arithmetic (`shared/calculators/`, `/calculators/*`,
  copilot `run_calculator`). They remain illustrative/preliminary over assumed
  inputs; verified-source figures are C2.
