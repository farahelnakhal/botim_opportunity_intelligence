...
| H2 | this branch | External-content ingestion hardening for R9a's social adapters, built **offline/adversarial-only** against the gated (not-live) Apple/Reddit adapters (PR-H2a..d). **PR-H2a:** adversarial prompt-injection suite (`test_social_content_injection.py`) proving a hostile review/post body is inert **data** across adapter→runner→extraction (the gap: strong data-never-instructions precedents existed for web pages + extraction, none for the social adapters). **PR-H2b:** PII **redact-and-flag** at the runner ingestion boundary for `pii_sensitive` providers — the deterministic floor was lifted from `merchant-voice/app/redaction.py` into **`shared/redaction.py`** (one impl; MV re-imports via a shim, its `test_redaction.py` passes unmodified); title/excerpt redacted before storage/model exposure, `quality_signals.pii_redaction`/`pii_categories`/`pii_manual_review` recorded, **fail-closed** (unredactable → content withheld); web-search path untouched. **PR-H2c:** bounded polite delay **between** a social adapter's internal HTTP calls + before its single retry (a gap the pass uncovered: the runner throttled only page fetches), wired on by the production builders, bare construction delay-free. **PR-H2d:** translation-fidelity is a **no-op** (R9a is querying-only; content translation deferred to R9c) — locked by a verbatim-storage tripwire. ⚠️ **H2 does NOT clear the R9a privacy/security review** — the live-ingestion gate (`RESEARCH_ALLOW_LIVE_SOCIAL`) stays closed; H2 hardening is proven now and simply begins applying when the human review clears it |
...
- **Deterministic calculators now exist (C1)** — request-time market-sizing /
  ...
  **preliminary/illustrative** over assumed inputs; binding inputs to verified
  sources (TAM/SAM/SOM from `RSRC-` ids) is **C2**, not built.
