...
| R6 | this branch | Scheduled monitoring re-run + email-on-change. Per-chat `workspace_subscriptions` + a multi-recipient child table in the workspace store (schema v3→v5) with **double opt-in** (hashed, 48h `MONITORING_CONFIRM_TTL_HOURS` confirm tokens; no mail until confirmed, enforced at the persistence layer) and **deterministic signed unsubscribe links** (`MONITORING_UNSUBSCRIBE_SIGNING_KEY`, RFC 8058-style, nothing stored per row). External-cron tick `POST /api/monitoring/tick` (`.github/workflows/monitoring-tick.yml`, hourly; shared secret `MONITORING_TICK_TOKEN`; idempotent claim-and-advance; skip-if-running) calls the SAME `build_workspace` orchestrator with `trigger='monitoring'`. `shared/email/` (pure-stdlib `smtplib` seam, MockEmailSender in tests, honest unconfigured no-op) + `monitoring_digest.py` email confirmed recipients ONLY on a **material change** (normalized-claim-**text** diff layered on `compare_versions` so `RCAND-` id churn can't spam; composite ≥0.01; degraded/no-change/failed send nothing, recorded honestly; overclaim guard fails safe). Per-user daily quota scaled by active subscriptions (`QUOTA_MONITORING_WORKSPACE_RUN_PER_DAY`, surfaced as `quota_used`/`quota_limit`); Analysis-tab opt-in/cadence/quota UI; `render.yaml` declares all R6 env. Nothing writes `knowledge-base/`; changed items stay preliminary. HuggingFace deploy path removed (Render only). ✅ **Live SMTP validation complete (2026-07-21):** all three `scripts/smoke_smtp.py` cases (real send; wrong-password → 502; unset vars → 503) plus the live double-opt-in confirmation flow and a real material-change digest with a working signed unsubscribe link were verified against a real inbox — closing R6's last known-unverified item (SPF/DKIM/domain reputation remain the operator's ongoing responsibility) |
| C1 | this branch | Deterministic calculators: `shared/calculators/` (pure typed-step engine — `base.py`/`calculators.py`/`render.py` — with a per-compute self-consistency check, F/E/A provenance + worst-of label propagation, raw-vs-display rounding split, honest never/undefined; 9 calculators incl. top-down + bottom-up `market_sizing` TAM/SAM/SOM and a `payments_take` not-gross-MDR/not-an-issuer guard) + a re-derivable `CALC-` owner-scoped store (`store.py`, `CALCULATORS_DB_PATH`; persists envelope + `calculator_version` + inputs). Executive API `GET /calculators`, `POST /calculators/{name}[/compute]`, `GET/DELETE /calculators/results` (owner-scoped, `calculator_save` quota). Copilot `run_calculator`/`list_calculators` tools + `deterministic_calculation` intent + grounding (shown-working facts, low confidence, no-decision) + a wordguard numeric-fidelity guard (the model narrates the tool's numbers, never computes; missing inputs → asks, never invents). Calculators UI panel + `lib/calculatorsApi.ts`. Contract `shared/contracts/calculators.schema.md`. The C2 `source_id` seam is reserved; nothing writes `knowledge-base/` |
| C2 | this branch | Verified-source market sizing (TAM/SAM/SOM) — computes market size from live, cited, tier-ranked sources, never invented or estimated (PR1 engine, PR2 composition+store+routes, PR3 UI+contracts+docs). **Corroboration** (`shared/research/corroboration.py`, pure): a quantity is `verified` iff ≥2 **independent** (distinct registrable domain) T1/T2 sources agree within a 10% relative tolerance band (`C2_CORROBORATION_TOLERANCE`); T3/T4 never count toward the ≥2 (Statista is T3 as an aggregator, so a primary + Statista is one voice, not two); representative value is the median of the agreeing set; otherwise `low_confidence` — stored and shown, never dropped or silently upgraded, never identical to a verified figure. **Figure extraction** (`shared/research/figures.py`): the model only proposes `{quantity,value,unit,source_id,quote}`; a figure persists only if exact-substring verified against the cited source AND the **verbatim-value guard** holds (the value's own digits appear in the quote — "1.2 billion"→`1.2`, never `1200000000`; no model expansion/rounding/recompute). Persisted on the research store as `RFIG-` (research **schema v7**). **Composition** (`executive-ui/api/market_sizing_builder.py`, deterministic, no model at build time): corroborates a run's figures and wires them into C1's `market_sizing`/`market_sizing_bottomup`, populating each sourced input's `source_id` and its H3 label — corroborated→**F**, low-confidence→**A**, analyst fractions→**A** (so a low-confidence figure is never identical to a corroborated one inside C1's own label propagation, not merely on a badge; review status is a separate axis). A sourced input with no figures refuses the build (422, quantity named) — never invented. **Candidate store** (`shared/market_sizing/`, `MSZ-`, `MARKET_SIZING_DB_PATH`): a sizing starts `pending_review`; a human approves/rejects exactly once (409 if terminal); approval records reviewer/note/timestamp and **nothing else** — no committed score, no `knowledge-base/` write, no EV id (that stays the `impact` CLI). Owner-scoped (indistinguishable 404s; legacy NULL shared). Routes `POST /opportunities/{OPP-nnn}/market-sizing` (mode-gated to demo corpus, quota-guarded, run owner-guarded), `POST /market-sizing/{id}/review`, `GET /market-sizing[/{id}]`, `GET /research/runs/{id}/figures`, `POST …/extract-figures` (honest 400 with no provider). UI: `MarketSizingSection` nested in the Research run detail (tier-badged figures, build form, candidate review) + `lib/marketSizingApi.ts`; verified vs low-confidence and Fact vs Assumption render as distinctly-worded, distinctly-classed badges (asserted at the DOM level). Contracts `research.schema.md` **v7** (figures) + new `market-sizing.schema.md` **v1**. Nothing writes `knowledge-base/` |
| R9a | this branch | Customer-research source adapters + source tiering + multi-language querying (PR9a-1..4). **Source tier** (`shared/research/source_tier.py`): human-curated domain→`T1..T4` registry (registry lookup only, never LLM/content-inferred; unknown→`T4`), derived at ingestion and stored on each source (research-store **schema v5**, existing rows backfilled); shared with C2's corroboration rule. **Apple App Store reviews adapter** (`AppStoreReviewsProvider`, public RSS, no key; per-review URL is constructed→`url_synthesized` + `im:rating` captured, both as `quality_signals`; UI shows "open linked page (not a direct link)" + a rating badge). **Reddit adapter** (`RedditProvider`, official API, app-only client-credentials OAuth, injectable; real permalinks→not synthesized). **Provider registry** (`build_provider`) + a **per-adapter fail-closed gate** (R9a privacy/security review done 2026-07-22): the old all-or-nothing `RESEARCH_ALLOW_LIVE_SOCIAL` is **retired**; `appstore` is **cleared** for live use behind its own opt-in `RESEARCH_ALLOW_LIVE_APPSTORE=1` (only the exact `1`), while `reddit` is **hard-blocked in code** — refused by `from_env` regardless of any env value (its commercial Data API terms are not cleared), so no operator toggle exists. **Multi-language querying** (`profiles.py`): human-curated per-language query templates (never machine-translated), `en`+`ar` first-class on `sme-financial-product`, tagged on each query (research-store **schema v6** `research_queries.language`); querying only (content=R9c). Contract `research.schema.md` bumped to v6. ✅ **Apple's privacy/security review is DONE** — App Store reviews RSS is public/unlicensed and cleared (see decision log 2026-07-22). ⚠️ **Reddit stays OPEN/gated/unresolved** — its Data API needs a paid commercial-partner agreement; hard-blocked until a separate decision. ⚠️ **`hi`/`ur`/`ml`/`tl` querying is recognized but not yet curated** (honest error if requested) — tracked follow-up. All adapters offline-tested (injected network); Apple is the only live-reachable social adapter, and only when explicitly opted in |
| R10 | this branch | Evidence-gap-driven merchant research-question generation (PR10a–c), **proposals only**. **PR10a:** `impact/gap_profile.py` — deterministic read-only per-opportunity weakest-link profiler (no-supporting-evidence, assumption-capped, contradicted, stale >180d, open gaps; reuses `impact/gaps.py`'s shown heuristic), contract `evidence-gap-profile.schema.md`, copilot tool `get_evidence_gap_profile`, `GET /api/opportunities/{OPP}/gap-profile` (mode-gated). **PR10b:** `executive-ui/api/question_generator.py` LLM-drafts questions from the profile (model proposes text/purpose/type; deterministic validation disposes — an invented `ASM-` link or a question failing Merchant Voice's own `validate_question_input` taxonomy is dropped, never softened; missing model → honest empty draft), persisted as `RQSET-` draft sets in the owner-scoped runtime store `shared/questions/store.py` (`QUESTION_SETS_DB_PATH`); routes `POST /opportunities/{OPP}/question-sets` (`question_generate` quota), `GET /question-sets[/{id}]`; contract `question-sets.schema.md`. **PR10c:** human review — store `review()` (draft→approved\|rejected once; schema v2 adds `reviewed_at`/`reviewer`/`review_note`) + `delete()`; routes `POST /question-sets/{id}/review` (reviewer edits **re-validated against MV's taxonomy** in the route layer), `GET /question-sets/{id}/handoff` (approved-only; copy-paste markdown + a MV-guide-shaped payload), `DELETE /question-sets/{id}`; "Research questions" frontend panel (`QuestionSetsPanel.tsx` + `lib/questionSetsApi.ts`). **Hard boundary (D3):** R10 never calls Merchant Voice, writes `knowledge-base/`, mints an EV id, changes a score, or contacts a merchant — a human creates the MV guide from a reviewed set through MV's own flow. Rebased onto `main` (post-R9a) for integration; R9a and R10 touch disjoint module trees (`shared/research/` vs `shared/questions/`) |
...
| P1 | this branch | Executive-brief **PDF export** (server-rendered). Pure `executive-ui/api/report_pdf.py` renders the SAME brief read models (`serialize.brief_payload` / `user_brief_payload`) to PDF bytes — recomputes/fetches/invents nothing; mirrors `impact/brief.py`'s view→renderer pattern. Every `Report.tsx` honest-state distinction is preserved (colored classification/confidence/archived labels; per-evidence freshness bands, stale keeps its ⚠, unknown labelled; persistent "no decision" banner; critical flags; contradictory-vs-supporting; "candidate — not repository evidence" on MV + external-research rows); empty sections render the web report's exact honest note, never omitted. All payload text XML-escaped; assembled prose routed through `impact/email.py`'s overclaim/bounded guard — an overclaim RAISES `ReportPdfError` (hard, honest failure, no partial PDF). Route `GET /brief/{id}/pdf` (both aliases) reuses the same serializer + OPP demo-gating + UOPP-always semantics, `application/pdf` + `Content-Disposition: attachment`; renderer error → 500 JSON `{error}` (never a blank 200). "Download PDF" button on the report (`api.briefPdfUrl`). **First runtime third-party dependency: reportlab 5.0.0 (BSD-3-Clause), pinned in `requirements.txt` + one Dockerfile pip line** — a deliberate logged decision (decision-log "P1 PDF export") that also sets the cost/risk-comparison bar for future dependency asks |
...
- Reports: web at `/report/{OPP-nnn|UOPP-…}` (refresh/direct-nav safe), plus a
  **server-rendered PDF** of the same brief (P1) via a "Download PDF" button →
  `GET /brief/{id}/pdf` (same read model, same gating; renders every honest-state
  distinction, never a client-side print hack).
...
| `CALCULATORS_DB_PATH` / `QUOTA_CALCULATOR_SAVE_PER_DAY` | C1 saved-calculation runtime SQLite (default `runtime/calculators.db`, gitignored) / per-user daily save cap (default 100, required-auth mode only; stateless compute is unquota'd) |
...
Phase C1 adds `shared/tests/{test_calculators,test_calculators_store}.py` (43
cases), `executive-ui/api/tests/test_calculator_routes.py` (9),
`copilot-backend/tests/test_calculator_tool.py` (13), and
`executive-ui/web/src/components/CalculatorsPanel.test.tsx` (3). Full-suite
tallies at the C1 tip: shared **271**, executive-ui/api **167**,
copilot-backend **144**, frontend Vitest **159**.

Phase C2 adds `shared/tests/{test_corroboration,test_figures,test_market_sizing_store,test_research_figures_store}.py`
and `executive-ui/api/tests/test_market_sizing.py` (builder + route lifecycle),
plus `executive-ui/web/src/components/MarketSizingSection.test.tsx` (which
asserts the verified/low-confidence and Fact/Assumption distinction at the
rendered-DOM level — different text AND class, side by side). Full-suite tallies
at the C2 tip: shared **361**, executive-ui/api **181**, copilot-backend **144**,
frontend Vitest **168**; the 10-step integration gate passes.
...
- **Executive-brief PDF *export* now ships (P1)** — server-rendered, via
  reportlab (the repo's sole runtime dependency). The remaining PDF gap is on
  *input*: uploaded-PDF parsing is still an honest 415 (.txt/.md/.csv/.docx
  only, R7) — a separate, still-open dependency decision.
...
- **Deterministic calculators now exist (C1)** — request-time market-sizing /
  unit-economics calculators with fully shown working, exposed to chat
  (`run_calculator`) and the API (`/calculators/*`), no LLM arithmetic. Still
  **preliminary/illustrative** over assumed inputs.
- **Verified-source market sizing now exists (C2)** — TAM/SAM/SOM composed from a
  run's corroborated, tier-ranked figures (source `source_id` on each C1 input),
  as `pending_review` `MSZ-` candidates a human approves/rejects. Limitations: a
  single global 10% tolerance band (no per-quantity/looser bands); build is
  gated to the demo corpus (attaches to committed `OPP-nnn`); figure extraction
  needs a configured provider; sizings are not yet surfaced in copilot chat or
  exported to PDF; no competitor benchmarking. Approval never writes committed
  scores or `knowledge-base/`.
