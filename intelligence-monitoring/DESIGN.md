...
| **P1 — External adapters** | adapter interface + registry + the **manual-intake adapter** shipped; **regulator-watch automated adapter shipped** (injected-network, RSS/Atom parse, relevance-gated scoring, graceful degradation — `scan --adapters regulator-watch`); remaining automated fetchers (app-store → newsroom/RSS → pricing differ) build behind the same interface, precision-first | ◑ FRAMEWORK + FIRST FETCHER DONE; more fetchers pending |
