...
`manual-intake` (shipped) is the lawful transport for human/LLM-collected observations: `tools/monitoring_engine/adapters.py`.
`regulator-watch` (shipped, P1's first automated fetcher) is `tools/monitoring_engine/adapter_regulator.py` — run opt-in via `monitor.py scan --adapters regulator-watch`. It is the reference implementation of this contract: network injected for offline testing, RSS/Atom parsing with no XML dependency, relevance-gated scoring (off-topic → insignificant → archived), automation-never-critical (urgency fixed low until a human/analyze pass upgrades it), graceful degradation on a dead source, and feed content treated strictly as data. Copy its shape for the next fetchers (app-store, newsroom, pricing differ).
